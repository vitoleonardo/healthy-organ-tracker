...
def dice_coef_single_label(class_idx: int, name: str, smooth=1) -> Callable[[tf.Tensor, tf.Tensor], tf.Tensor]:
    def dice_coef(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        y_true_f = K.flatten(y_true_single_class)
        y_pred_f = K.flatten(y_pred_single_class)
        intersection = K.sum(y_true_f * y_pred_f)
        return (2. * intersection + smooth) / (K.sum(y_true_f) + K.sum(y_pred_f) + smooth)
...
def iou_coef(y_true, y_pred, smooth=1):
    ...
    (IoU) between them
    3
    :param y_true: Ground truth mask3
